...
describe MonsoonIdentity::SessionStore do
  
  describe 'initialize' do
    it "should create a new session_store object" do 
      expect(MonsoonIdentity::SessionStore.new({})).not_to be(nil)
    end
  end
  ...
  describe "session store object" do    
    describe "token_presented?" do
      context "token is presented" do
        let(:session) { MonsoonIdentity::SessionStore.new({ monsoon_identity_token: ApiStub.keystone_token.merge(expires_at:(Time.now+1.day).to_s )}) }
      ...
      context "session is empty" do
        let(:session) { MonsoonIdentity::SessionStore.new({ }) }
    ...
    describe "token_eql?" do
      let(:session) { MonsoonIdentity::SessionStore.new({ monsoon_identity_token: ApiStub.keystone_token.merge(value:ApiStub.keystone_token["value"])}) }
    ...
    describe "token" do
      let(:session) { MonsoonIdentity::SessionStore.new({ monsoon_identity_token: ApiStub.keystone_token}) }

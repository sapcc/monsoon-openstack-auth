...
describe MonsoonOpenstackAuth::Controller, :type => :controller do
  ...
  context "ignore empty scope" do
    ...
    it "authentication should ignore empty organization and project" do
      expect(MonsoonOpenstackAuth::Session).to receive(:check_authentication).with(controller,'europe', organization: nil, project: nil, raise_error:nil)
      get 'index', region_id: 'europe'
    end
  ...
  context "scope not nil" do
    ...
    it "authenticate with scope 0-12345 and p-12345" do
      expect(MonsoonOpenstackAuth::Session).to receive(:check_authentication).with(controller,'europe', organization: "o-12345", project: "p-12345", raise_error:nil)

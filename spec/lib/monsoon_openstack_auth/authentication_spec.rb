...
describe MonsoonOpenstackAuth::Authentication, :type => :controller do
  ...
  context "api_authentication_required" do
    ...
    it "should raise a not_authorized_error if not authenticated" do
      ...
      expect { get 'index', domain_id: 'aaa', project_id: 'bbb' }.to raise_error(MonsoonOpenstackAuth::Authentication::NotAuthorized)
    end
  ...
  describe 'two factor parameter' do
    context 'two_factor parameter is true' do
      controller do # anonymous subclass of ActionController::Base
        authentication_required domain: -> c {c.params[:domain_id]}, project: -> c {c.params[:project_id]}, two_factor: true
      ...
      it "should set two_factor parameter to true" do
        expect(MonsoonOpenstackAuth::Authentication::AuthSession).to receive(:check_authentication).with(controller, domain: nil, domain_name: nil, project: nil, raise_error:nil, two_factor:true).and_return(nil)
        get :index
      end
    end
    ...
    context 'two_factor parameter is a method' do
      controller do # anonymous subclass of ActionController::Base
        authentication_required domain: -> c {c.params[:domain_id]}, project: -> c {c.params[:project_id]}, two_factor: :two_factor
    ...
    context 'two_factor parameter is a proc' do
      controller do # anonymous subclass of ActionController::Base
        authentication_required domain: -> c {c.params[:domain_id]}, project: -> c {c.params[:project_id]}, two_factor: -> c {false}
      ...
      it "should set two_factor parameter to true" do
        expect(MonsoonOpenstackAuth::Authentication::AuthSession).to receive(:check_authentication).with(controller, domain: nil, domain_name: nil, project: nil, raise_error:nil, two_factor:false).and_return(nil)
        get :index
      end
    end
  end
